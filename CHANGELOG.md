...
- 核心原语（已实现并单测）：
  ...
  - `lib/yaml.mjs`、`lib/ledger.mjs`、`lib/scan.mjs`、`lib/paths.mjs`。
- 工具面（`lib/tools.mjs`）：`migrate_codex_preview`、`migrate_codex_mcp` 已实现；其余为占位工具（返回友好报告）。
- CLI（`bin/codex2dsh.mjs`）：`preview` / `mcp` / `ledger`。
- 测试：34 个 `node:test` 用例全绿；脱敏夹具（`test/fixtures/`）；CI（GitHub Actions，node 22/24 × ubuntu/windows + 夹具敏感词检查）。

### 待实现（见 docs/08-路线图.md）

- M2 余项：`migrate_codex_skills` 落盘。

...
export async function importMemoryToMnemon(args = {}) {
  ...
  if (existsSync(summaryPath)) {
    ...
    // 容量裁剪留安全余量：插件统计口径比投影字节略大（实测 +30~60 B），
    // 直接按 limit 裁剪会在插件侧超限；余量 128 B 保证不超
    const userTrim = trimMnemonEntries(user, MNEMON_RUNTIME_LIMITS.user - 128)
    const memTrim = trimMnemonEntries(memory, MNEMON_RUNTIME_LIMITS.memory - 128)
  ...
  for (const [fileName, title] of Object.entries(MNEMON_DOC_TITLES)) {
    ...
    // relativePath 必须带 documents/ 前缀（插件 pathFor 校验 managed 根，缺前缀 = unsafe managed path）
    record.relativePath = `documents/active/${record.filename}`

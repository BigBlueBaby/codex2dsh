...
export function buildMcpPlan(parsedConfig, opts = {}) {
  ...
  for (const [name, server] of Object.entries(parsedConfig.mcpServers ?? {})) {
    ...
    const entry = {
      ...
      // 兜底字符串化：TOML 数值参数（如 --port 54322）在 DSH args schema 中必须是 string
      args: maskedArgs.args.map((v) => String(v)),
      env: maskedEnv.env,
 ...
 * 契约（@deepseek-ai/dsh-mcp-client 实测）：每台服务器一个插件实例——
 *   - insert:
 *       - id: mcp-<serverName>        # 每个 entry id 唯一
 *         name: '@deepseek-ai/dsh-mcp-client'
 *         config:
 *           serverName: <serverName>  # 必填 namespace（[A-Za-z0-9_-]{1,32}）
 *           transport: stdio          # 或 streamable-http（不是 Codex 的 type）
...
export function renderMcpPlan(input) {
  ...
  L.push('# 每台服务器一个 dsh-mcp-client 实例（serverName/transport 契约见 @deepseek-ai/dsh-mcp-client）')
  if (input.plan.length === 0) {
    L.push('# 无待迁移服务器')
  }
  for (const s of input.plan) {
    L.push('- insert:')
    L.push(`    - id: mcp-${s.name}`)
    // name 必须是 loader 可解析的完整包名（不是短 id）——cordis-plugin-loader 按
    // name 解析安装包；@ 开头裸标量在 YAML 中非法，必须加引号
    L.push("      name: '@deepseek-ai/dsh-mcp-client'")
    L.push('      config:')
    L.push(`        serverName: ${renderScalar(s.name)}`)
    if (String(s.type).toLowerCase() === 'http') {
      L.push('        transport: streamable-http')
      L.push(`        url: ${renderScalar(s.url ?? '')}`)
    } else {
...
/**
 * 从 mcp-mirror.cordis.yml 提取服务器名（行级扫描，零依赖）。
 * 结构（renderMcpPlan 输出，每台服务器一个 dsh-mcp-client entry）：
 *   - insert:
 *       - id: mcp-google-mcp-toolbox
 *         name: '@deepseek-ai/dsh-mcp-client'
 *         config:
 *           serverName: google-mcp-toolbox   ← 以此行提取服务器名
 *           transport: stdio
 * @param {string} mirrorPath
 * @returns {string[]} 服务器名（去重；文件缺失/不可读返回空数组）
 */
export function readMirrorServerNames(mirrorPath) {
  ...
  for (const line of text.split(/\r?\n/)) {
    const m = /^\s*serverName:\s*(.+?)\s*$/.exec(line)
    if (m) {

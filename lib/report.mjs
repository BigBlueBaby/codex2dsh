 ...
 *   裸字符串不处理（无法判定语义）。
 * @param {string[]} args
 * @param {object} [opts] { mask?: boolean } 默认 true（脱敏）；false = 原样迁移（仅计数）
 * @returns {{ args: string[], maskedCount: number }} maskedCount 恒为敏感值数量（供报告）
 */
export function maskArgs(args, opts = {}) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    if (eq && isSecretKey(eq[1])) {
      if (!isSafeValue(eq[2])) maskedCount++
      out.push(eq[1] + (mask ? maskValue(eq[2]) : eq[2]))
      continue
    }
    const sp = FLAG_SPACE_RE.exec(item)
    if (sp && isSecretKey(sp[1]) && list[i + 1] !== undefined) {
      const next = list[i + 1]
      if (!isSafeValue(next)) maskedCount++
      out.push(item, mask ? maskValue(next) : next)
      i++
      continue
    }
    ...
    if (env && isSecretKey(env[1])) {
      if (!isSafeValue(env[2])) maskedCount++
      out.push(env[1] + '=' + (mask ? maskValue(env[2]) : env[2]))
      continue
    }
    out.push(item)
  }
 ...
 * @param {object} [opts] { mask?: boolean } 默认 true（脱敏）；false = 原样保留（仅计数）
 * @returns {{ env: Record<string, string>, maskedCount: number }}
 */
export function maskEnv(env, opts = {}) {
  const mask = opts.mask !== false
  let maskedCount = 0
  const out = {}
  for (const [k, v] of Object.entries(env ?? {})) {
    if (isSecretKey(k)) {
      if (!isSafeValue(v)) maskedCount++
      out[k] = mask ? maskValue(v) : v
    } else {
      out[k] = v
    }
  }

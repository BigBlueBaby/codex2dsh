...
import { resolveCodexHome, resolveMcpMirrorPath } from './paths.mjs'
...
export function registerTools(ctx, ledgerDir) {
  ...
  ctx.tools.register(defineTool({
    ...
    description: '把 ~/.codex/skills 的用户技能转换落盘为 DSH 技能资产（$DSH_AGENTS_HOME/skills，默认 ~/.agents/skills）；默认 dry-run。',
    parameters: {
      apply: { type: 'boolean', description: 'true 时落盘；缺省 false = 预览'},
      codexHome: { type: 'string'},
      agentsHome: { type: 'string', description: '可选：技能落盘根（默认 $DSH_AGENTS_HOME）'},
      fixFrontmatter: { type: 'boolean'},
      force: { type: 'boolean'},
    },
    output: toolOutput(),
    async execute() {
      return notImplemented('migrate_codex_skills', 'M2')()
    },
  }))
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      apply: { type: 'boolean'},
      codexHome: { type: 'string'},
      agentsHome: { type: 'string'},
    },
    output: toolOutput(),
    async execute() {
      return notImplemented('migrate_codex_instructions', 'M3')()
    },
  }))

  ctx.tools.register(defineTool({
    name: 'migrate_codex_memory',
    description: '把 Codex 记忆（memories/ 与 memories_1.sqlite）导出为 DSH 记忆资产；默认 dry-run；sqlite 不可读时降级报告。',
    parameters: {
      apply: { type: 'boolean'},
      codexHome: { type: 'string'},
      outDir: { type: 'string'},
    ...
    output: toolOutput(),
    async execute() {
      return notImplemented('migrate_codex_memory', 'M3')()
    },
  }))

  ctx.tools.register(defineTool({
    name: 'migrate_codex_config',
    description: '只读生成模型/Provider/权限/项目信任迁移建议（settings-suggest.yml），绝不自动修改 settings.yaml。',
    parameters: {
      codexHome: { type: 'string'},
      outPath: { type: 'string'},
    },
    output: toolOutput(),
    async execute() {
      return notImplemented('migrate_codex_config', 'M3')()
    },
  }))
  ...
  ctx.tools.register(defineTool({
    ...
    output: toolOutput(),
    async execute() {
      return notImplemented('codex2dsh_doctor', 'M3')()
    },
  }))

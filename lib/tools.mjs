...
export function registerTools(ctx, ledgerDir) {
  ...
  // ── migrate_codex_sessions（M4，已实现：统计 + 委托 import_codex）─────────
  ctx.tools.register(defineTool({
    name: 'migrate_codex_sessions',
    description: '统计 ~/.codex/sessions 会话规模并委托 import_codex（dsh-chat-import）导入为可续聊的 DSH 会话；未安装 dsh-chat-import 时返回安装指引。',
    parameters: {
      preview: { type: 'boolean', description: 'true 时只统计与委托预览（不落盘）'},
      codexHome: { type: 'string'},
      budget: { type: 'integer', description: '可选：上下文预算（token 数），透传给 import_codex'},
      restamp: { type: 'boolean', description: '可选：导入会话时间戳平移到当前时间，透传给 import_codex'},
    },
    output: toolOutput(),
    async execute(args) {
      const a = args ?? {}
